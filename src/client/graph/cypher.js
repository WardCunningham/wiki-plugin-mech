export function parse(text, log = () => {}) {
  const r = {},
    x = {} // rules defined and traced
  let left = '',
    right = text // text parsed and pending
...
export function apply(graph, code) {
  ...
  function chain(node, code, maybe) {
    if (
      (!code.node.type || node.type == code.node.type) &&
      (!code.node.prop || node.props[code.node.prop[0]] == code.node.prop[1])
    ) {
      if (code.node.bind) maybe[code.node.bind] = node
      if (code.chain.rel) {
        if (['in', 'both'].includes(code.chain.rel.dir)) links(node.in, 'from')
        if (['out', 'both'].includes(code.chain.rel.dir)) links(node.out, 'to')
      } else {
        results.push(maybe)
      }
    }
    ...
    function links(rids, dir) {
      rids.forEach(rid => {
        if (
          (!code.chain.rel.type || rels[rid].type == code.chain.rel.type) &&
          (!code.chain.rel.prop || rels[rid].props[code.chain.rel.prop[0]] == code.chain.rel.prop[1])
        ) {
          maybe = { ...maybe }
          if (code.chain.rel.bind) maybe[code.chain.rel.bind] = rels[rid]
