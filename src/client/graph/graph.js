...
export class Nodes {
  ...
  i(type = null, props = {}) {
    ...
    let rids = this.nids
      .map(nid => this.graph.nodes[nid].in)
      .flat()
      .filter(uniq)
  ...
  o(type = null, props = {}) {
    ...
    let rids = this.nids
      .map(nid => this.graph.nodes[nid].out)
      .flat()
      .filter(uniq)
    if (type) rids = rids.filter(rid => this.graph.rels[rid].type == type)
    for (const key in props) rids = rids.filter(rid => this.graph.rels[rid].props[key] == props[key])
  ...
  props(key = 'name') {
    ...
    return this.nids
      .map(nid => this.graph.nodes[nid].props[key])
      .filter(uniq)
      .sort()
  }
  ...
  types() {
    return this.nids
      .map(nid => this.graph.nodes[nid].type)
      .filter(uniq)
      .sort()
  }
...
export class Rels {
  ...
  props(key = 'name') {
    ...
    return this.rids
      .map(rid => this.graph.rels[rid].props[key])
      .filter(uniq)
      .sort()
  }
  ...
  types() {
    return this.rids
      .map(rid => this.graph.rels[rid].type)
      .filter(uniq)
      .sort()
  }
